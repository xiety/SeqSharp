...
let private renderLifeLine (lifeLine: VisualLifeLine) state =
    ...
    {
        LifeLineX = state.LifeLineX + LifelineStep + (float32(lifeLine.Depth) - 1.0f) * (StepSize * 0.5f)
...
let renderSvg input =
    ...
    let styles = ["svg { background-color: white; }"
                  "rect { fill:#eeeeee; fill-opacity:1; stroke-width:1; stroke:gray; }"
