...
let private safeString (line: string) =
    line.Replace(">", "&gt;").Replace("<", "&lt;");

let private make el =
    match el with
    | Rect (x, y, width, height, title, cls) -> sprintf "<rect x='%f' y='%f' width='%f' height='%f' class='%s'><title>%s</title></rect>" x y width height cls (safeString title)
    | Line(x1, y1, x2, y2, cls) -> sprintf "<line x1='%f' y1='%f' x2='%f' y2='%f' class='%s' />" x1 y1 x2 y2 cls
    | Text(x, y, text, cls) -> sprintf "<text x='%f' y='%f' class='%s'>%s</text>" x y cls (safeString text)
